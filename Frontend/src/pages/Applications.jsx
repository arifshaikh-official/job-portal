            ...
            isEdit || userData && userData.resume === ""
              ? <>
                <label className='flex items-center' htmlFor="resumeUpload">
                  <p className='bg-blue-100 text-gray-600 px-4 py-2 rounded-lg mr-2'>{resume ? resume.name : "Select Resume"}</p>
              ...
              : <div className='flex gap-2'>
                <a target='_blank' href={userData.resume} className='bg-blue-100 text-gray-600 px-4 py-2 rounded-lg'>Resume</a>
          ...
          <tbody>
            {userApplications.map((job, index) => true ? (
              <tr key={index} className='border border-gray-300'>
                ...
                <td className='py-3 px-4 max-sm:hidden'>{moment(job.date).format('ll')}</td>
